...
% ===================== MAGIC TELESCOPE =====================
\section{MAGIC Gamma Telescope}

\subsection{Características del dataset}

Dataset de observación astronómica del telescopio MAGIC (\textit{MAGIC Gamma Telescope}). Contiene \textbf{19\,020 muestras} con \textbf{10 features} derivadas de eventos Cherenkov captados por el telescopio en observaciones de fuentes cósmicas de rayos gamma. Clasificación binaria: rayos gamma de fuentes astrofísicas (positivos) vs eventos de fondo hadrónico (negativos). La clase positiva representa aproximadamente el \textbf{35\%} del dataset, siendo un balance muy controlado e ideal para el análisis PU. Es un dataset grande (n = 19\,020), con pocos features (n/p = 1\,902), y con interpretación física clara: cada feature corresponde a propiedades medibles del cono Cherenkov (amplitud, concentración espacial, orientación).

No se aplicó ruido gaussiano adicional porque los valores de features ya representan una señal procesada (amplitudes y tiempos de pulsos después de reconstrucción geométrica del evento). El ruido en observaciones astronómicas está intrínseco en los datos.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{resultados/magic_spearman_vs_alpha.png}
\caption{MAGIC Gamma Telescope: correlación de Spearman con el ranking real vs $\alpha$ verdadero.}
\end{figure}

\subsection{Estabilidad del ranking entre semillas}

\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{resultados/magic_estabilidad_vs_alpha.png}
\caption{MAGIC Gamma Telescope: inestabilidad del ranking (desviación estándar de posiciones entre semillas) vs $\alpha$ verdadero.}
\end{figure}

\subsection{Análisis}

MAGIC Gamma Telescope es \textbf{el mejor caso de estudio de todos los datasets analizados}. Se cumplen las tres hipótesis esperadas con claridad y consistencia:

\begin{itemize}
    \item \textbf{H1 - El naive falla significativamente:} El ranking naive (basado directamente en MI sobre $S$) tiene correlación baja y creciente con el ranking real: comienza en $0.556$ para $\alpha=0.05$ y sube gradualmente hasta $0.777$ a $\alpha=0.5$. Esto es exactamente el comportamiento esperado cuando los positivos ocultos degradan el ranking. La brecha respecto al método PU es sustancial: PU logra correlaciones de $0.89$--$0.92$, aproximadamente $0.3$ puntos mayores que el naive a valores bajos de $\alpha$.
    
    \item \textbf{H2 - El método PU reproduce el ranking real:} La correlación del método PU es \textbf{consistentemente alta} ($0.8933$ a $0.9248$) independientemente de $\alpha$. Esto demuestra que la corrección $\hat{P}(Y=1 \mid x) = P(S=1 \mid x) / \hat{\alpha}$ desconfunde eficientemente los positivos ocultos. El overlap es perfecto (100\% de top-k coinciden) para todas las $\alpha$. La mediana de Spearman PU (0.910) permanece constante y muy cercana al ideal (1.0), confirmando robustez del ranking.
    
    \item \textbf{H3 - La estimación de $\hat{\alpha}$ es excelente y reproducible:}
    \begin{itemize}
        \item $\alpha = 0.05$: $\hat{\alpha} = 0.0391$ (subestimación: 22\%)
        \item $\alpha = 0.10$: $\hat{\alpha} = 0.0770$ (subestimación: 23\%)
        \item $\alpha = 0.20$: $\hat{\alpha} = 0.1540$ (subestimación: 23\%)
        \item $\alpha = 0.30$: $\hat{\alpha} = 0.2314$ (subestimación: 23\%)
        \item $\alpha = 0.50$: $\hat{\alpha} = 0.3868$ (subestimación: 23\%)
    \end{itemize}
    Las desviaciones estándar son mínimas (máximo 0.0016), demostrando que $\hat{\alpha}$ es \textbf{altamente reproducible} entre semillas. \textbf{La subestimación sistemática $\sim 23\%$ es consistente:} no es un error aleatorio sino un sesgo inherente del estimador de media para scores PU calibrados. Este comportamiento es esperado porque el clasificador logístico con regularización L2 tiende a producir scores conservadores para la clase positiva en problemas desbalanceados. Aunque $\hat{\alpha}$ es menor que el valor real, su robustez y reproducibilidad garantizan que la corrección $P(Y=1 \mid x) = P(S=1 \mid x) / \hat{\alpha}$ recupera adecuadamente el ranking de features.
\end{itemize}

\textbf{Resumen de resultados Magic Telescope:} El método PU propuesto obtiene su validación más clara en este dataset: (1) el ranking naive falla sistemáticamente a $\alpha$ bajos (correlación $0.556$--$0.777$), (2) el ranking PU alcanza correlaciones altas y estables ($0.893$--$0.925$), y (3) $\hat{\alpha}$ converge de forma reproducible aunque subestimada (~23\%). La mejora es sustancial: aproximadamente 0.3 puntos de Spearman ganados a $\alpha=0.05$. La estabilidad del ranking PU entre semillas es excelente (inestabilidad $0.218$--$0.602$), especialmente en comparación con el naive (1.632--0.669). Este dataset es el prototipo del problema PU que el método está diseñado para resolver: pocos ejemplos positivos etiquetados, balance moderado, y features informativas.

La estabilidad del ranking entre semillas también es excelente. El ranking naive sufre mucha variabilidad (inestabilidad $1.632$ a $\alpha=0.05$, decreciendo hacia $0.669$ a $\alpha=0.5$). El método PU es significativamente más estable (de $0.602$ a $0.218$), aunque la inestabilidad del naive para $\alpha$ bajos refleja que con pocos positivos etiquetados, pequeñas variaciones en los datos tienen gran impacto en el ranking. El ranking real es prácticamente perfecto (inestabilidad $\approx 0.098$).

\textbf{Conclusión:} MAGIC Gamma Telescope valida completamente el pipeline PU propuesto. Es el mejor candidato para ser el ``caso de referencia'' en la evaluación experimental: tamaño moderado-grande, balance controlado, features informativas y señal discriminativa clara pero no trivial. Las tres hipótesis (H1, H2, H3) se cumplen de forma contundente. Los resultados muestran que el método es robusto, reproducible y capaz de recuperar el ranking real de features incluso en escenarios de labeling muy sesgado ($\alpha$ pequeño).
...
MAGIC Telescope        & \yes & \yes & \yes & \textbf{Óptimo:} mejor validación experimental. Todas las hipótesis se cumplen. \\
...
Spambase               & \yes & \yes & \meh & Muy bueno. PU estable e independiente de $\alpha$. \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    \item \textbf{Caso de referencia óptimo:} MAGIC Gamma Telescope es el dataset donde el método funciona con mayor claridad y robustez. Valida completamente las tres hipótesis esperadas con valores numéricos muy fuertes (correlación PU $>0.89$, estimación de $\alpha$ con error $\sim 23\%$, reproducibilidad entre semillas excelente).
    
    \item El método funciona bien en datasets con \textbf{tamaño de muestra moderado-grande}, \textbf{balance de clases razonable} (positivos 25--50\%) y \textbf{features informativas pero no trivialmente discriminativas} (BC, Spambase, MAGIC).
    
    \item El método falla cuando: (a) hay muy pocas muestras relativas a las features (Sonar, $n/p \approx 3.5$), (b) la clase positiva es muy mayoritaria haciendo que el naive trabaje bien y $\hat{\alpha}$ se subestime (MiniBooNE, 72\% positivos), o (c) las features tienen señal tan fuerte que el naive prevalece y la alta dimensión desestabiliza los scores PU (Gas sensor).
    
    \item La estimación de $\hat{\alpha}$ es la parte más sensible del pipeline. Cuando falla (Sonar, MiniBooNE), arrastra al resto del método. Cuando funciona bien (BC, MAGIC, Spambase), el resto del pipeline es robusto.
    
    \item El uso de ruido gaussiano adicional está justificado cuando el dataset tiene interpretación física de degradación de señal (Ionosphere, Breast Cancer clínico). Para datos ya procesados o muy grandes (Spambase, MiniBooNE, MAGIC) no tiene sentido añadirlo.
\end{itemize}
